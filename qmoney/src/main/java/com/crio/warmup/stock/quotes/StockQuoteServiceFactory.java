...
public enum StockQuoteServiceFactory {
  ...
  public StockQuotesService getService(String provider,  RestTemplate restTemplate) {
    if (provider != null && provider.equalsIgnoreCase("tiingo"))  {
      StockQuotesService tingQuotesService = new TiingoService(restTemplate);
      return tingQuotesService;
    }
    StockQuotesService alphaQuotesService = new AlphavantageService(restTemplate);
    return alphaQuotesService;
  }
}
